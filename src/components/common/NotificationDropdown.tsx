...
const NotificationDropdown: React.FC<NotificationDropdownProps> = React.memo(({
  ...
  // Memoize icon selection to avoid recalculation on every render
  const getNotificationIcon = useCallback((type: string, message: string) => {
    if (!message) return Info;
    
    const lowerMessage = message.toLowerCase();
    if (lowerMessage.includes("upload")) return Upload;
    if (lowerMessage.includes("message")) return MessageSquare;
    if (
    ...
    return notifications.map((notification) => {
      ...
      return (
        <motion.div
          ...
          } border-l-4 ${getPriorityColor((notification as { priority?: string }).priority)}`}
  ...
  }, [notifications, getNotificationIcon, getNotificationColor, getPriorityColor, handleNotificationClick, deleteNotification]);
